...
from src.langgraphagenticai.tools.google_map_review import (  # noqa: E402
...
def get_googlereviews(restaurant_name: str) -> dict:
    ...
    This function fetches detailed Google Maps reviews including ratings, comments, and author information.
    Args:
        restaurant_name (str): The name of the restaurant
    Returns:
        dict: The reviews for the given restaurant name including:
            ...
            - phone: Phone number
            - website: Website URL
            - reviews: List of individual reviews with author, rating, comment, and time
    """
    try:
...
@mcp.tool()
def get_restaurant_reviews(restaurant_name: str) -> dict:
    """
    Get Google Maps reviews for a specific restaurant by name.
    This is the main function to get restaurant reviews from Google Maps.
    Args:
        restaurant_name (str): The name of the restaurant (e.g., "Sasou", "Sushi Saito")
    Returns:
        dict: Complete restaurant information with reviews including:
            - restaurant_name: Name of the restaurant
            - address: Full address
            - rating: Overall rating (1-5 stars)
            - total_reviews: Total number of reviews
            ...
            - phone: Phone number
            - website: Website URL
            - opening_hours: Opening hours for each day
            - reviews: List of individual reviews with author, rating, comment, and time
    """
    try:
        result = get_reviews_for_restaurant(restaurant_name)
        if result is None:
            return {
                "error": f"Could not find reviews for {restaurant_name}. Please check the restaurant name."
            }
        return result
    except Exception as e:
        return {"error": str(e)}
                ...
                results.append(
                    {
                        ...
                        "price_level": restaurant.get("priceSummary", {}).get(
                            "priceRangeLevel", "Unknown"
                        ),
                    }
                )
