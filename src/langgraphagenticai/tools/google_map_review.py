...
def search_nearby_places(
    lat: float, lng: float, radius: int, place_type: str = "restaurant"
) -> Dict:
    """
    ...
    try:
        ...
        places = gmaps.places_nearby(
            location=(lat, lng), radius=radius, type=place_type, keyword="sushi"
        )

        return {"results": places.get("results", []), "status": places.get("status")}
...
def get_reviews_for_restaurant(restaurant_name: str) -> Optional[Dict]:
    ...
    try:
        ...
        if not place_id:
            raise ValueError(
                f"Could not find place_id for restaurant: {restaurant_name}"
            )
